...
class ConvertOrchestrator:
    ...
    def clean_state(self, book_dirs: list[Path]) -> None:
        ...
        if cleaned_books or cleaned_work:
            log.info(f"Cleaned state: {cleaned_books} book records, {cleaned_work} work dirs")
    ...
    def run_batch(self, book_dirs: list[Path]) -> BatchResult:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            while queued or active:
                ...
                while queued and len(active) < max_workers and cpu_load < self.config.cpu_ceiling:
                    ...
                    log.debug(f"Submitted {book_path.name} (threads={threads}, active={len(active)}/{max_workers})")
                    # Let CPU readings stabilize before submitting the next job
                    time.sleep(2.0)
                    cpu_load = psutil.cpu_percent(interval=1.0)

                # Display status
                self._display_status(cpu_load, len(active), len(queued), completed, failed)

                # Wait for at least one job to complete
                if active:
                    done, pending = wait(active.keys(), timeout=5.0, return_when=FIRST_COMPLETED)
        ...
        return BatchResult(completed=len(completed), failed=len(failed), total=len(book_dirs))
        ...
        for stage in mvp_stages:
            ...
            if stage_status == StageStatus.COMPLETED.value and not self.config.force:
                if self._is_stage_stale(book_hash, stage, source_path):
                    ...
                    log.debug(f"Skipping {stage.value} for {source_path.name} (already completed)")
                    continue
            ...
            if stage == Stage.CONVERT:
                ...
                log.debug(f"Convert stage for {source_path.name} using {threads} threads")
            ...
            if stage == Stage.METADATA and self._retag_existing_destination(book_hash, source_path):
                break
    ...
    def _is_stage_stale(self, book_hash: str, stage: Stage, source_path: Path) -> bool:
        ...
        if stage == Stage.VALIDATE:
            ...
            if not audio_list.exists():
                log.warning(f"Stale: audio_files.txt missing for {source_path.name}, re-running from validate")
                return True

        elif stage == Stage.CONCAT:
            files_txt = work_dir / "files.txt"
            if not files_txt.exists():
                log.warning(f"Stale: files.txt missing for {source_path.name}, re-running from concat")
                return True

        elif stage == Stage.CONVERT:
            output_file = self.db.read_field(book_hash, "stages.convert.output_file") or self.db.read_field(
                book_hash, "metadata.output_file"
            )
            if not output_file or not Path(output_file).is_file():
                log.warning(f"Stale: convert output missing for {source_path.name}, re-running from convert")
                return True
            ...
            if output_file and not Path(output_file).is_file():
                log.warning(f"Stale: convert output missing for {source_path.name}, re-running metadata")
                return True
    ...
    def _retag_existing_destination(self, book_hash: str, source_path: Path) -> bool:
        ...
        log.info(f"Destination file exists: {existing_file.name} -- re-tagging in place, skipping organize")
        ...
        click.echo(f"  Re-tagged in place: {existing_file.name} (artist={metadata['author']!r})")
    ...
    def _calculate_max_workers(self) -> int:
        ...
        if self.config.max_parallel_converts > 0:
            ...
            log.debug(f"Auto-calculated max_workers: {max_workers} (cpu_count={cpu_count})")
        return max_workers
    ...
    def _display_status(
        ...
        """
        status = f"  [CPU: {cpu_load:.0f}%] active={active} queued={queued} done={len(completed)} failed={len(failed)}"
        click.echo(f"\r{status}", nl=False)

    def _display_summary(self, completed: list[Path], failed: list[Path], total: int) -> None:
        ...
        """
        click.echo("\n")  # Clear status line
        click.echo(f"\nBatch conversion complete: {len(completed)}/{total} succeeded, {len(failed)} failed")
