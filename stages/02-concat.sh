...
# Reads audio_files.txt from stage 01, produces files.txt + metadata.txt for stage 03.
...
stage_concat() {
  ...
  local audio_list="$WORK_DIR/audio_files.txt"
  if [[ ! -f "$audio_list" ]]; then
    die "audio_files.txt not found -- run stage 01 first"
  fi

  local FILE_COUNT
  FILE_COUNT=$(wc -l < "$audio_list" | tr -d ' ')

  # Generate ffmpeg concat demuxer file list
  # Escape apostrophes for ffmpeg concat: ' -> '\'' (end quote, literal quote, restart quote)
  local concat_file="$WORK_DIR/files.txt"
  : > "$concat_file"
  while IFS= read -r audio_file; do
    local escaped
    escaped=$(printf '%s' "$audio_file" | sed "s/'/'\\\\''/g")
  ...
  while IFS= read -r audio_file; do
    local duration_s
    duration_s=$(get_duration "$audio_file")
