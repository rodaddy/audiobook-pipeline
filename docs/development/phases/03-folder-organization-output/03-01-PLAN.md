...
must_haves:
  ...
  key_links:
    ...
    - from: "lib/organize.sh:build_plex_path"
      to: "WORK_DIR/audnexus_book_*.json"
      via: "jq extraction of author/title/series/year from cached Audnexus JSON"
      pattern: "audnexus_book_.*\\.json"
...
- Input: $1 = base_dir, $2 = work_dir, $3 = book_hash, $4 = source_path
- **Primary metadata source:** Read from Audnexus JSON cache in WORK_DIR (`audnexus_book_*.json`) via jq.
  This is the same JSON that stage 06 uses for tagging. Extract:
  - author: `.authors[0].name`
  - title: `.title`
  - series_name: `.seriesPrimary.name` (may be null)
  - series_position: `.seriesPrimary.position` (may be null)
  - year: `.releaseDate` (extract year from ISO date)
- **Fallback when no Audnexus JSON** (METADATA_SKIP=true or no ASIN):
  - Author: basename of source_path parent directory -> "Unknown Author"
  - Title: basename of source_path -> "Unknown Title - HASH"
  - Series: omit folder entirely
  - Year: omit from folder name
...
- Locate M4B from manifest `stages.convert.output_file` (same path metadata stage uses at line 35 of 06-metadata.sh)
- Read metadata via build_plex_path() which reads from Audnexus JSON cache in WORK_DIR (primary) or source folder name (fallback)
- Call check_nfs_available() before NFS operations (5s timeout)
...
- Add to STAGE_MAP: `[organize]="07"` (keep cleanup at "04" -- Plan 03-02 handles renumbering)
- Update STAGE_ORDER: `(validate concat convert asin metadata organize cleanup)`
- Add source line after lib/metadata.sh: `source "$LIB_DIR/organize.sh"`
- Add defaults: `NFS_OUTPUT_DIR="${NFS_OUTPUT_DIR:-$OUTPUT_DIR}"` and `CREATE_COMPANION_FILES="${CREATE_COMPANION_FILES:-true}"`
- Export: `export NFS_OUTPUT_DIR CREATE_COMPANION_FILES`
...
Note: Cleanup stays at "04" and stages/04-cleanup.sh is NOT renamed. Plan 03-02 will add archive at "08", rename cleanup to "09", and modify cleanup behavior to skip M4B output (organize handles it).
  </action>
  <verify>
