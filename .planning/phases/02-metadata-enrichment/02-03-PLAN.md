...
must_haves:
  ...
  artifacts:
    ...
    - path: "bin/audiobook-convert"
      provides: "Updated pipeline with stage 05 in STAGE_MAP and STAGE_ORDER"
      contains: "metadata"
...
Output: `stages/05-metadata.sh` (new stage), `lib/metadata.sh` (tone wrapper library), updated pipeline entry point and manifest schema.
</objective>
...
<task type="auto">
  <name>Task 1: Create lib/metadata.sh and stages/05-metadata.sh</name>
  <files>lib/metadata.sh, stages/05-metadata.sh</files>
  <action>
...
**stages/05-metadata.sh** -- Follow the exact same structure as `stages/03-convert.sh`:
...
4. Read ASIN from manifest: `manifest_read "$BOOK_HASH" "stages.asin_discovery.asin"` (set by 02-01 stage). If empty, `log_warn "No ASIN found -- skipping metadata enrichment"` and set stage completed with `enriched: false`, return 0
...
**Graceful degradation is critical.** Only `tone tag` failure should cause stage failure (return 1). All other failures (no ASIN, no API data, no cover, no chapters, duration mismatch) should log warnings and continue. The stage should exit 0 even without any metadata -- the M4B is still valid from Phase 1.
  ...
  <verify>
- `bash -n stages/05-metadata.sh` exits 0 (syntax valid)
- `bash -n lib/metadata.sh` exits 0 (syntax valid)
- `shellcheck stages/05-metadata.sh` passes (no errors, warnings acceptable)
- `shellcheck lib/metadata.sh` passes
- `grep -c 'tone tag' lib/metadata.sh` returns at least 1
- `grep -c 'log_warn' stages/05-metadata.sh` returns at least 3 (graceful degradation paths)
...
- `grep 'include-property' lib/metadata.sh` confirms safe JSON dump (no binary data)
  ...
  <done>
- stages/05-metadata.sh exists with stage_metadata() following the same pattern as stages/03-convert.sh
...
<task type="auto">
  <name>Task 2: Wire stage 05 into pipeline and update manifest schema</name>
...
2. Add `metadata` to the STAGE_MAP associative array:
   ...
   [metadata]="05"
   ```
3. Insert `metadata` into STAGE_ORDER array between `convert` and `cleanup`:
   ```bash
   STAGE_ORDER=(validate concat convert metadata cleanup)
   ```
   This ensures metadata tagging happens AFTER conversion (needs M4B) but BEFORE cleanup (cleanup moves file to output).
...
1. In `manifest_create()`, add `metadata` stage to the initial JSON template:
   ...
   ```
   Place it after `convert` and before `cleanup` in the stages object.
2. In `get_next_stage()`, add `metadata` to the for loop's stage list:
   ```bash
   for stage in validate concat convert metadata cleanup; do
   ```
...
- Stage order in STAGE_ORDER array is: validate, concat, convert, metadata, cleanup (metadata before cleanup)
  </verify>
  <done>
- bin/audiobook-convert sources lib/metadata.sh, includes metadata in STAGE_MAP as "05", and STAGE_ORDER has metadata between convert and cleanup
...
<verification>
1. `bash -n stages/05-metadata.sh && bash -n lib/metadata.sh && bash -n bin/audiobook-convert && bash -n lib/manifest.sh` -- all pass syntax check
2. `shellcheck stages/05-metadata.sh lib/metadata.sh` -- no errors
3. `grep -A1 'STAGE_ORDER' bin/audiobook-convert` -- shows metadata between convert and cleanup
...
7. Dry-run test: `./bin/audiobook-convert --dry-run /path/to/test-book/` -- pipeline runs through all 5 stages without error (metadata stage skips gracefully due to no ASIN in dry-run)
</verification>

<success_criteria>
- New stage 05 (metadata) exists and integrates into the 5-stage pipeline: validate -> concat -> convert -> metadata -> cleanup
