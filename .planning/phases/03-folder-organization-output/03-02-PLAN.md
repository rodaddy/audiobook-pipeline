...
must_haves:
  truths:
    - "M4B passes 6-point validation (exists, ffprobe parses, duration > 0, codec AAC, container mov/mp4, file size within 10%) before originals archived"
    - "Archive stage dies with clear error if M4B validation fails -- originals preserved"
    - "Original MP3 files moved to ARCHIVE_DIR/book-basename/ only after successful validation"
    - "Manifest records archive_path, archived_at, and original_count"
    - "Cleanup stage (renumbered 09) only cleans work directory -- does NOT copy M4B to OUTPUT_DIR (organize handles output)"
  artifacts:
    - path: "lib/archive.sh"
      provides: "M4B validation and archive file operations"
      exports: ["validate_m4b_integrity", "archive_originals"]
    - path: "stages/08-archive.sh"
      provides: "Archive stage orchestration with validation gate"
      exports: ["stage_archive"]
    - path: "stages/09-cleanup.sh"
      provides: "Renumbered cleanup stage (was 04), work directory cleanup only"
    - path: "bin/audiobook-convert"
      provides: "Pipeline with full stage order"
      contains: "[archive]=\"08\""
  key_links:
    - from: "stages/08-archive.sh"
      to: "lib/archive.sh"
      via: "validate_m4b_integrity() and archive_originals() function calls"
    - from: "lib/archive.sh"
      to: "lib/ffmpeg.sh"
      via: "get_duration(), get_codec() for M4B validation"
    - from: "stages/08-archive.sh"
      to: "stages.organize.output_path (manifest)"
      via: "manifest_read to locate organized M4B for validation"
---
...
3. **Rename and modify cleanup stage:**
   ...
   - **Remove M4B output logic** from cleanup stage: delete the block that copies M4B to OUTPUT_DIR (lines 45-76 of current 04-cleanup.sh). The organize stage (07) now handles M4B output to NFS_OUTPUT_DIR.
   - Cleanup should only: (a) mark overall status as "completed" in manifest, (b) clean WORK_DIR if CLEANUP_WORK_DIR=true
   - Keep the manifest status update (`.status = "completed"`, `.completed_at`) but remove `.stages.cleanup.output_path` and `.stages.cleanup.file_size` since organize stage records those
